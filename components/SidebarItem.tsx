...
const SidebarItem = ({
  ...
  return (
    <div
      onClick={handleClick}
      className={`relative flex flex-row justify-start items-center gap-4 text-xl mx-2 px-4 py-2 hover:bg-gray-1/50 rounded-full md:rounded-md md:object-fit hover:cursor-pointer ${
        activePage === activeSegment ? "font-semibold" : "font-normal"
      }`}
    >
      {activePage === activeSegment ? (
        <ActiveIcon size={32} className="text-primary-dark" />
      ) : (
